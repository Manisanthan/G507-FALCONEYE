...
parser.add_argument('--desired_feature', default=[], action='append', help='The feature we wish to detect and track from the annotated feature')
parser.add_argument('--desired_height', default=240, type=int, help='desired_height resolution')
parser.add_argument('--desired_width', default=320, type=int, help='desired_width resolution')
...
parser.add_argument('--save_images_to', default=False, help='The path to save all segmentation/tracking frames')
...
parser.add_argument('--metric', default='closest_mean', help='Not supported on all mode, leave as default')
...
parser.add_argument('--fly_meters', default=0, type=int, help='meter to fly at the beginning. If 0 then drone should be already flying')
...
# Global variables (to be replaced with explicit state passing)
mission_counter = 0
postion_vector_queue = queue.Queue(30)
FREQ = 0.5
    ...
    if cfg['detect'] == 'clip':
        ...
        for file_name in os.listdir(cfg['queries_dir']):
            if file_name.endswith(".pt"):
                ...
                queries[key] = query
        if not queries.keys():
            print(f"No annotations found in {cfg['queries_dir']}!!!!, see step 1 and script annotate_features.py")
            exit(1)
...
def get_siammask_tracker(siam_cfg, device):
    ...
    if args.siam_tracker_model:
        assert os.path.isfile(args.siam_tracker_model), f'Please download {args.siam_tracker_model} first.'
        siammask = load_pretrain(siammask, args.siam_tracker_model)
    siammask.eval().to(device)
    return siammask

def plot_similarity_if_needed(cfg, frame, similarity_rel, alpha=0.5):
    if cfg['plot_visualizations'] or cfg["save_images_to"]:
        ...
        plot_and_save_if_needed(cfg, _overlay, "DINO-CLIP-result", 0)

def plot_and_save_if_needed(cfg, image_to_plot, stage_and_task, count, multiply=1):
    ...
    if cfg['save_images_to']:
        file_name = f"{cfg['save_images_to']}/{stage_and_task}/{mission_counter}_{count}.jpg"
...
def automatic_object_detection(vit_model, sam, video, queries, cfg, vehicle):
    ...
    with torch.cuda.amp.autocast():
        while not detected:
            ...
            while not read_one_frame:
                ...
                if not read_one_frame:
                    ...
                    if frame_fail_count >= max_frame_fail:
                        print(f"[ERROR] Unable to read frames from camera/video after {max_frame_fail} attempts. Exiting.")
                        exit(1)
                    time.sleep(0.1)
                else:
                    frame_fail_count = 0
            ...
            plot_and_save_if_needed(cfg, frameshow, 'Stream_segmenting', count)

            if cfg['use_sam']:
                t = time.time()
                _, masks = sam.seg(frameshow)
                print(f"Sam took: {time.time() - t:.2f}s")
                ...
                plot_and_save_if_needed(cfg, masks_of_sam, 'SAM-result', count, multiply=255)

            t = time.time()
            if cfg['detect'] in ["dino", "image"]:
            ...
            if cfg['use_sam']:
                ...
                for ii, mask in enumerate(masks):
                    ...
                    m = mask['segmentation']
                    if cfg['detect'] == 'clip':
                        if ii == 0 or mask['area'] < float(cfg['min_area_size']):
                            continue
                    ...
                    if len(queries.items()) > 1:
                        if cfg['detect'] == 'clip' and ii == 0:
                            continue
                        mask_label = torch.argmax(torch.as_tensor(mask_similarity))
                        ...
                        if float(mask_similarity[0]) > float(cfg['class_threshold']):
                            thresh[m] = 1
            else:
                if len(queries.keys()) == 1:
                    ...
                    for _idx, query in queries.items():
                        if cfg['metric'] == 'closest_mean':
                            ...
                            class_similarity = []
                            for single_annotation in query:
                                single_annotation = torch.nn.functional.normalize(single_annotation.cuda(), dim=0)
                                similarity = cosine_similarity(img_feat_norm, single_annotation.view(1, -1, 1, 1))
                    ...
                    class_labels = torch.argmax(similarities, dim=0)[0]
            ...
            if not cfg['dont_allow_contours_mix'] or not cfg['use_sam']:
                ...
                for label_for_detected_obj in range(1, marker_count):
                    ...
                    if area > cfg['min_area_size']:
                        ...
                        print(f"[DETECTION] Bounding box coordinates: x={x}, y={y}, w={w}, h={h}")
                        if cfg['plot_visualizations'] or cfg['save_images_to']:
                            cv2.rectangle(dino_plot, (x, y), (x + w, y + h), (255, 0, 0), 3)
                            cv2.rectangle(frameshow, (x, y), (x + w, y + h), (255, 0, 0), 3)
                            cv2.putText(dino_plot, f"x:{x} y:{y} w:{w} h:{h}", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
                            cv2.putText(frameshow, f"x:{x} y:{y} w:{w} h:{h}", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
                    if not mask.any():
                        break
            else:
            ...
            count += 1
            if len(detections) > 0:
                plot_and_save_if_needed(cfg, dino_plot, "DINO-CLIP-result", count, multiply=255)
                plot_and_save_if_needed(cfg, frameshow, "Detection", count)
                detecton_count += 1
                if detecton_count == 1 and not cfg['detect_only']:
                    print(f"Found {len(detections)} desired objects! Moving to tracking")
                    return detections, all_masks.astype(float), saved_frame
            torch.cuda.empty_cache()
            gc.collect()
            print(f"Time took: {time.time() - s:.2f}s")

def compute_area_and_center(bounding_shape):
    x = 0
    y = 1
    bounding_shape = bounding_shape[0]
...
def track_object_with_siammask(siammask, detections, video, cfg, tracker_cfg, vehicle):
    ...
    print(f"Initial bounding box: x={x}, y={y}, w={w}, h={h}")
    toc = 0
    f = 0
    object_lost_counter = 0
    max_object_lost_frames = 10

    while True:
        ret, im = video.read()
        if not ret:
            object_lost_counter += 1
            if object_lost_counter >= max_object_lost_frames:
                print("Object lost for too long. Redetecting...")
                start_mission(device, tracker_cfg, cfg, siammask, detector, segmentor, queries, video, vehicle)
                return
            continue
        object_lost_counter = 0
        ...
        if f == 0:
            ...
            state = siamese_init(im, target_pos, target_sz, siammask, tracker_cfg['hp'], device=device)
        elif f > 0:
            ...
            x_min = np.min(bounding_shape[:, 0, 0])
            y_min = np.min(bounding_shape[:, 0, 1])
            x_max = np.max(bounding_shape[:, 0, 0])
            y_max = np.max(bounding_shape[:, 0, 1])
            x, y, w, h = x_min, y_min, x_max - x_min, y_max - y_min
            print(f"[TRACKING] Bounding box coordinates: x={x}, y={y}, w={w}, h={h}")
            cv2.rectangle(im, (x, y), (x + w, y + h), (0, 255, 0), 2)
            cv2.putText(im, f"x:{x} y:{y} w:{w} h:{h}", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            cv2.imshow('Tracker-result', im)
            if cfg['save_images_to']:
                cv2.imwrite(f"{cfg['save_images_to']}/Tracker-result/{f}.jpg", im)
            key = cv2.waitKey(cfg['wait_key'])
            if key > 0:
                break
            if cfg['save_images_to']:
                cv2.imwrite(f"{cfg['save_images_to']}/Stream_tracking/{f}.jpg", im_store)
        f += 1
        toc += cv2.getTickCount() - tic
    toc /= cv2.getTickFrequency()
    fps = f / toc
    print(f'SiamMask Time: {toc:.1f}s Speed: {fps:.1f}fps (with visualization!)')

def create_video_from_images(cfg):
    vfile = f'{cfg["path_to_video"]}/video_from_images.avi'
    fileidx = 0
        ...
        if cfg['video_order'] == 'any':
            ...
            while True:
                ...
                if not os.path.exists(filename):
                    filename = os.path.join(cfg['path_to_video'], f"1_{fileidx}.jpg")
                if not os.path.exists(filename):
                    break
                img = cv2.imread(filename)
                height, width, layers = img.shape
                ...
                img_array.append(img)
                fileidx += 1
        out = cv2.VideoWriter(vfile, cv2.VideoWriter_fourcc(*'DIVX'), 15, size)
...
def init_system():
    ...
    if cfg['tracker'] == 'siammask':
        ...
        print(f"Exiting. No such tracker {cfg['tracker']}")
        exit(9)
    ...
    if cfg['detect'] == 'clip':
        ...
        print(f"Exiting. No such detector {cfg['detect']}")
        exit(9)
    ...
    if isinstance(video_source, int):
        ...
        print(f"Making video from images in directory {cfg['path_to_video']}")
        video = create_video_from_images(cfg)
    elif os.path.exists(cfg["path_to_video"]) and cfg['fps'] < 1:
        print(f"Reading video {cfg['path_to_video']}")
        video = cv2.VideoCapture(cfg["path_to_video"])
    else:
        print(f"Using live camera stream from {cfg['path_to_video']}")
        video = cv2.VideoCapture(cfg["path_to_video"])
    ...
    if cfg['save_images_to']:
        ...
        for directory_to_create in ['SAM-result', 'Stream_segmenting', 'DINO-CLIP-result', 'Tracker-result', 'Stream_tracking', 'Detection']:
            os.makedirs(os.path.join(cfg['save_images_to'], directory_to_create), exist_ok=True)
...
def compute_drone_action_while_tracking(mean_point, cfg, vehicle):
    global mean, postion_vector_queue
    x = 1
    y = 0
    z = 2
    heading = 0
    if mean_point is None:
        ...
        loop.run_until_complete(move_drone_by_velocity(vehicle, 0, 0, 0, 0, K=0, yaw_K=0))
        return
    if cfg['use_yaw']:
    ...
    normalized_center[x] = mean_point[x] / cfg['desired_width']
    normalized_center[y] = mean_point[y] / cfg['desired_height']
    ...
    if postion_vector_queue.full():
        p_filt = filtfilt(cof_b, cof_a, np.array(list(postion_vector_queue.queue)), axis=0)
        postion_vector_queue.get()
        loop.run_until_complete(move_drone_by_velocity(vehicle, p_filt[-1][0], p_filt[-1][1], 0, p_filt[-1][2], K=1.5, yaw_K=0.15))
    else:
        loop.run_until_complete(move_drone_by_velocity(vehicle, direction_vector[0], direction_vector[1], 0, direction_vector[2], K=1.5, yaw_K=0.15))
...
def track_object_with_aot(tracker, pred_mask, frame, video, cfg, vehicle, track_single_object=True):
    ...
    with torch.cuda.amp.autocast():
        while True:
            ...
            plot_and_save_if_needed(cfg, frame, "Stream_tracking", frame_idx)
            plot_and_save_if_needed(cfg, vis_masks, 'Tracker-result', frame_idx, multiply=255)
            coords = np.where(pred_mask == 1)
            if len(coords[0]) > 0:
                y_min, y_max = coords[0].min(), coords[0].max()
                x_min, x_max = coords[1].min(), coords[1].max()
                x, y, w, h = x_min, y_min, x_max - x_min, y_max - y_min
                ...
                cv2.putText(frame, f"x:{x} y:{y} w:{w} h:{h}", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                cv2.imshow('Tracking', frame)
            print(f"processed frame {frame_idx}, obj_num {tracker.get_obj_num()}", end='\r')
            frame_idx += 1
            read_one_frame = False
            while not read_one_frame:
                read_one_frame, frame = video.read()
    ...
    while True:
        ...
        if state > 1:
            if cfg['tracker'] == 'siammask':
                ...
                masks, _, _ = sam.predict(box=input_box, multimask_output=False)
                cap.release()
                cv2.destroyAllWindows()
                return input_box, masks, frame

def detect_object(cfg, detector, segmentor, video, queries):
    print(f"Applying {cfg['detect']} detection...")
    bounding_boxes, masks_of_sam, saved_frame = None, None, None
    if cfg['detect'] in ['dino', 'clip', 'image']:
        bounding_boxes, masks_of_sam, saved_frame = automatic_object_detection(
            vit_model=detector, sam=segmentor, video=video, queries=queries, cfg=cfg, vehicle=vehicle)
        ...
        if masks is not None:
            ...
            plot_and_save_if_needed(cfg, vis_masks, 'Choose_object', 0)
        else:
            masks_of_sam = None
    return bounding_boxes, masks_of_sam, saved_frame

def start_mission(device, tracker_cfg, cfg, tracker, detector, segmentor, queries, video, vehicle, mission_counter=0):
    mission_counter += 1
    ...
    if cfg['tracker'] == "siammask":
        ...
        if status == 'FAILED':
            print("Redetecting....")
            start_mission(device, tracker_cfg, cfg, tracker, detector, segmentor, queries, video, vehicle, mission_counter)
