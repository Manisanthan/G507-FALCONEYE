...
def seg_track_anything(input_video_file, aot_model, sam_gap, max_obj_num, points_per_side):
    ...
    with torch.cuda.amp.autocast():
        while cap.isOpened():
            ...
            if frame_idx == 0:
              ...
              SegTracker.add_reference(frame, SegTracker.refined_merged_mask)  # <-- Add this
              frame_idx += 1
              continue

            elif (frame_idx % sam_gap) == 0:
                # seg_mask, _ = segtracker.seg(frame)
                torch.cuda.empty_cache()
                gc.collect()
                ...
                # ONLY track the original object
    ...
    fourcc = cv2.VideoWriter_fourcc(*"mp4v")  # Always use mp4v for .mp4 output
...
def tracking_objects_in_video(SegTracker, input_video):
    ...
    with torch.cuda.amp.autocast():
        while cap.isOpened():
            ...
            if frame_idx == 0:
                pred_mask = SegTracker.refined_merged_mask
                SegTracker.add_reference(frame, pred_mask)
            elif (frame_idx % sam_gap) == 0:
                pred_mask = SegTracker.track(frame, update_memory=False)
                obj_id = 1
                pred_mask = (pred_mask == obj_id).astype(np.uint8) * obj_id
            else:
                pred_mask = SegTracker.track(frame, update_memory=False)
                obj_id = 1
                pred_mask = (pred_mask == obj_id).astype(np.uint8) * obj_id
    ...
    if not out.isOpened():
        print("---")
        print("!!! ERROR: cv2.VideoWriter failed to open.")
        print("!!! This usually means the 'avc1' codec is not supported.")
        print("!!! Please run the fix in Step 2.")
        print("---")
        # Return empty paths to avoid crashing Gradio further
        return None, None
